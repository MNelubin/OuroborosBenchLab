...
# [OUROBOROSBENCH] hook — injected by OuroborosBench patcher
_BENCH_ON_TOOL_CALL   = None  # set by bench_cli.py if needed
_BENCH_ON_TOOL_RESULT = None  # set by bench_cli.py if needed

def _bench_hook_tool_call(name, input_data):
    if _BENCH_ON_TOOL_CALL:
        try:
            _BENCH_ON_TOOL_CALL(name, input_data)
        except Exception:
            pass

def _bench_hook_tool_result(name, output):
    if _BENCH_ON_TOOL_RESULT:
        try:
            _BENCH_ON_TOOL_RESULT(name, output)
        except Exception:
            pass

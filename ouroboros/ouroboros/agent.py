...
class OuroborosAgent:
    ...
    def __init__(self, env: Env = None, event_queue: Any = None):
        if env is None:
            # Default environment for benchmark mode
            env = Env(repo_dir=pathlib.Path("/app"), drive_root=pathlib.Path("/transcripts"))
        self.env = env
    ...
    async def run_bench(self, task: str, logger, workspace: str):
        ...
        ctx = ToolContext(
            ...
            current_task_type="bench",
            emit_progress_fn=lambda x: None,
            task_depth=0,
            is_direct_chat=True,
        )
        ...
        try:
            ...
            import traceback
            traceback.print_exc()
            logger.log_assistant(f"Error: {e}")
            return "error"
    # =====================================================================
    # Main entry point
    # =====================================================================
...
# Compatibility alias for bench_cli.py
